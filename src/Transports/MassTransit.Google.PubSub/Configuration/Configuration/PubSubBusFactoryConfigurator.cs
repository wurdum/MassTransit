...
namespace MassTransit.Configuration
{
    ...
    public class PubSubBusFactoryConfigurator :
        BusFactoryConfigurator,
        IPubSubBusFactoryConfigurator,
        IBusFactory
    {
        ...
        public PubSubBusFactoryConfigurator(IServiceBusBusConfiguration busConfiguration)
        ...
        public void Send<T>(Action<IPubSubMessageSendTopologyConfigurator<T>> configureTopology)
            where T : class
        {
            IPubSubMessageSendTopologyConfigurator<T> configurator = _busConfiguration.Topology.Send.GetMessageTopology<T>();

            configureTopology?.Invoke(configurator);
        }

        public void Publish<T>(Action<IPubSubMessagePublishTopologyConfigurator<T>>? configureTopology)
            where T : class
        {
            IPubSubMessagePublishTopologyConfigurator<T> configurator = _busConfiguration.Topology.Publish.GetMessageTopology<T>();

            configureTopology?.Invoke(configurator);
        }

        public void Publish(Type messageType, Action<IPubSubMessagePublishTopologyConfigurator>? configure = null)
        {
        ...
        public new IPubSubSendTopologyConfigurator SendTopology => _busConfiguration.Topology.Send;
        public new IPubSubPublishTopologyConfigurator PublishTopology => _busConfiguration.Topology.Publish;

        public void Host(IPubSubHostSettings settings)
        {
            _busConfiguration.HostConfiguration.Settings = settings;
        }

        public void ReceiveEndpoint(IEndpointDefinition definition, IEndpointNameFormatter? endpointNameFormatter,
            Action<IPubSubReceiveEndpointConfigurator>? configureEndpoint)
        {
        ...
        public void ReceiveEndpoint(string queueName, Action<IPubSubReceiveEndpointConfigurator> configureEndpoint)
        {
